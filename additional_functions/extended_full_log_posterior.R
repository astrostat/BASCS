extended_full_log_posterior <- function(mu_curr,allocate_curr,w,k_curr,eparas,ewt,alpha){
  ...
  for (j in 1:k_curr){
    like_obs[,j] <- w[j]*psf_cpp(off.angle,ellip,slope,psf.norm,r0,spatial,mu_curr[,j])*(ewt[j]*dgamma(energy,eparas[j,3],eparas[j,3]/eparas[j,1])+(1-ewt[j])*dgamma(energy,eparas[j,4],eparas[j,4]/eparas[j,2]))
  }
